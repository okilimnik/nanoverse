(() => {
  ...
  // out/hydration_console/core.mjs
  var PX_PER_A = 42;
  var OH_LEN = 0.9572;
  ...
  var to_px = function(a) {
    return { "x": get(a, "x") * PX_PER_A, "y": get(a, "y") * PX_PER_A, "z": get(a, "z") * PX_PER_A };
  };
  ...
  var illo = new Zdog.Illustration({ "element": canvas, "zoom": 1.6, "dragRotate": true, "rotate": { "x": -0.28, "y": 0.55 } });
  var R = { "C": 15, "O": 16, "H": 9 };
  var COLOR = { "C": "#4b4f58", "O": "#e2555a", "H": "#eef1f4", "bond": "#9aa5b1", "hbond": "#59e0c9" };
  var element_of = function(k) {
    return keyword(subs(name(k), 0, 1));
  };
  var eth_group = new Zdog.Anchor({ "addTo": illo });
  for (let G__1 of iterable(eth_keys)) {
    let k11 = G__1;
    const el_2 = element_of(k11);
    new Zdog.Shape({ "addTo": eth_group, "translate": to_px(get(eth, k11)), "stroke": get(R, el_2), "color": get(COLOR, el_2) });
  }
  for (let G__1 of iterable(eth_bonds)) {
    let vec___212 = G__1;
    let a13 = nth(vec___212, 0, null);
    let b14 = nth(vec___212, 1, null);
    new Zdog.Shape({ "addTo": eth_group, "path": [to_px(get(eth, a13)), to_px(get(eth, b14))], "stroke": 6, "color": get(COLOR, "bond") });
  }
  var anchors = mapv((function(w) {
    const a_1 = new Zdog.Anchor({ "addTo": illo, "translate": to_px(get(w, "base-o")) });
    const local_2 = get(w, "local");
    new Zdog.Shape({ "addTo": a_1, "translate": to_px(get(local_2, "O")), "stroke": get(R, "O"), "color": get(COLOR, "O") });
    new Zdog.Shape({ "addTo": a_1, "translate": to_px(get(local_2, "H1")), "stroke": get(R, "H"), "color": get(COLOR, "H") });
    new Zdog.Shape({ "addTo": a_1, "translate": to_px(get(local_2, "H2")), "stroke": get(R, "H"), "color": get(COLOR, "H") });
    new Zdog.Shape({ "addTo": a_1, "path": [to_px(get(local_2, "O")), to_px(get(local_2, "H1"))], "stroke": 5, "color": get(COLOR, "bond") });
    new Zdog.Shape({ "addTo": a_1, "path": [to_px(get(local_2, "O")), to_px(get(local_2, "H2"))], "stroke": 5, "color": get(COLOR, "bond") });
    return a_1;
  }), waters);
  var tubes = mapv((function(_) {
    return new Zdog.Shape({ "addTo": illo, "path": [{ "x": 0, "y": 0, "z": 0 }, { "x": 0, "y": 0, "z": 0 }], "stroke": 5, "color": get(COLOR, "hbond"), "visible": false });
  }), pairs);
  ...
  var frame = function(now) {
    ...
    const lf_10 = (() => {
      const or__23663__auto___11 = last_frame_8;
      if (truth_(or__23663__auto___11)) {
        return or__23663__auto___11;
      } else {
        return now;
      }
      ;
    })();
    const dt_12 = Math.min((now - lf_10) / 1e3, 0.05);
    const t_accum_SINGLEQUOTE__13 = truth_(running_5) ? t_accum_9 + dt_12 * speed_7 : t_accum_9;
    const current_14 = mapv((function(_PERCENT_1) {
      return compute_current(_PERCENT_1, t_accum_SINGLEQUOTE__13, amplitude_6);
    }), waters);
    const active_count_15 = atom(0);
    const nearest_16 = atom(Infinity);
    swap_BANG_(state, assoc, "last-frame", now, "t-accum", t_accum_SINGLEQUOTE__13);
    const n15_17 = count(waters);
    let i_18 = 0;
    for (; i_18 < n15_17; i_18++) {
      nth(anchors, i_18).translate.set(to_px(get(nth(current_14, i_18), "O")));
    }
    ;
    const n16_19 = count(pairs);
    let idx_20 = 0;
    for (; idx_20 < n16_19; idx_20++) {
      (() => {
        const p_21 = nth(pairs, idx_20);
        const has_donor_22 = !(get(p_21, "donor") == null);
        const w_idx_23 = has_donor_22 ? get(p_21, "donor") : get(p_21, "acceptor");
        const c_24 = nth(current_14, w_idx_23);
        const H_25 = has_donor_22 ? get(p_21, "donor-atom") === "H1" ? get(c_24, "H1") : get(c_24, "H2") : get(eth, "H9");
        const D_26 = has_donor_22 ? get(c_24, "O") : get(eth, "O");
        const A_27 = has_donor_22 ? get(eth, "O") : get(c_24, "O");
        const dist_vec_28 = sub(A_27, H_25);
        const dist_29 = vlen(dist_vec_28);
        const vec_hd_30 = vnorm(sub(D_26, H_25));
        const vec_ha_31 = vnorm(dist_vec_28);
        const cos_angle_32 = vdot(vec_hd_30, vec_ha_31);
        const active_33 = dist_29 < DIST_LIMIT && cos_angle_32 < COS_LIMIT;
        const tube_34 = nth(tubes, idx_20);
        tube_34.visible = active_33;
        if (truth_(active_33)) {
          tube_34.path[0] = to_px(H_25);
          tube_34.path[1] = to_px(A_27);
          tube_34.updatePath();
          swap_BANG_(active_count_15, inc);
        }
        ;
        if (dist_29 < deref(nearest_16)) {
          return reset_BANG_(nearest_16, dist_29);
        }
        ;
      })();
    }
    ;
    readout_count.textContent = deref(active_count_15);
    readout_count.className = `num${(deref(active_count_15) === 0 ? " zero" : "") ?? ""}`;
    readout_dist.textContent = truth_(isFinite(deref(nearest_16))) ? `${deref(nearest_16).toFixed(2) ?? ""}${" \xC5"}` : "\u2014";
